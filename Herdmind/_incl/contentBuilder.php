...
{
   global $fandom; echo "<!-- fandom is ".$fandom[2]." -->";
	global $userName;
	global $userid;
	if (!$userName)
		$userName = $userid;
...
<HEADER>
	<H1>
		<A HREF=\"/\">
			Herdmind
			<SMALL CLASS=\"slogan\">The ".$fandom[2]." Headcanon Database</SMALL>
		</A>
	</H1>
	...
	if ($userName)
		...
		<UL ID=\"LOGGED_IN_USERNAV\">
			...
			<LI><A HREF=\"/profile/?fandom=".$fandom[0]."&id=$userid\" CLASS=\"button\">" . $userName . "</A></LI>
...
<NAV ID=\"SIDEBAR\">
	...
	echo "
		<LI CLASS=\"expandable\"><A HREF=\"/browse/\">Browse</A>
			<UL>
				<LI><A HREF=\"/browse/index.php?fandom=".$fandom[0]."&type=Character\"> Characters </A></LI>
				<LI><A HREF=\"/browse/index.php?fandom=".$fandom[0]."&type=Species\">   Species    </A></LI>
				<LI><A HREF=\"/browse/index.php?fandom=".$fandom[0]."&type=Place\">     Places     </A></LI>
				<LI><A HREF=\"/browse/index.php?fandom=".$fandom[0]."&type=Event\">     Events     </A></LI>
				<LI><A HREF=\"/browse/index.php?fandom=".$fandom[0]."&type=Object\">    Objects    </A></LI>
				<LI><A HREF=\"/browse/index.php?fandom=".$fandom[0]."&type=Other\">     Other      </A></LI>
			</UL>
		</LI>
...
{	
	...
	$ret = "
		<" . $tagName . ">
			<A HREF=\"http://herdmind.net/topic?fandom=".$fandom[0]."&t=" . $topic->pageid . "\" CLASS=\"topic\">
				<FIGURE>
			...
			<' . ($standalone ? 'DIV' : 'LI') .
				' CLASS="' . ($fact->uservote ? ($fact->uservote < 0 ? "down" : "up") . "voted " : "") . "fanfact" . ($classes ? " " . $classes : "") . "\" TABINDEX=\"-1\">
				 ...
				 <DIV CLASS=\"meta\">
					...
					($moreData ? "<A HREF=\"/fanfact?fandom=".$fandom[0]."&id=$fact->factid\" CLASS=\"callToAction\">More data</A>" : '') . "
...
//$Title = htmlentities($rs[0],ENT_QUOTES);
		...
		$string = str_replace("[p" . $child->pageid . "]","<div class=\"popname\">
			<a href=\"/topic?fandom=".$fandom[0]."&t=" . $child->pageid . "\" class=\"apagetitle\">" . $child->pagename . "</a><!-- These comments are here because there must be NO whitespace before, within, or after this fanfact, or else things like punctuation marks will not properly be placed flush with the fact
...
function OldPageCodePicker()
{
	...
	global $db_connection;
		...
		echo "<div class='left3' style='width:200px;' onclick='addcode(\"[p".$results[0]."]\")'><span class='spanleft'><div><img width='50' height='50' src='/".$results[1]."'></div></span><span class='spanright'>".$results[5]."
				...
				<sub>
				[p".$results[0]."]
				<a href='../topic.php?fandom=".$fandom[0]."&pageid=".$results[0]."' target='_blank' id='nonbutton'>View Page</a>
				</sub></span></div>";
	...
	if(isset($comments))
	{
		foreach($comments as &$comment)
		{
			...
			$pageEchoes .= "
				<LI>
					<FIGURE CLASS=\"avatar\">
						<A HREF=\"/profile/?fandom=".$fandom[0]."&id=".$comment->memberid.'">
			...
			$pageEchoes .= '
						<DIV CLASS="comment-text">	
							'.$comment->postbody."<br/>
							<a href=\"/$linkedpage?fandom=".$fandom[0]."&id=".$comment->topicid.'">'.$comment->timeposted.'</a>
						</DIV>
					</DIV>
				</LI>';
		}
